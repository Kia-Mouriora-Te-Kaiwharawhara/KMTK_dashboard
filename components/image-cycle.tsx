...
function ImageCycle({
    ...
    pairs: Array<{ image: string; text: string, link: string, credit: string, audio: string }>,
    header?: string
                ...
                <CarouselContent>
                    {pairs.map((pair, index) => (
                        <CarouselItem key={index}>
                                <a href={pair.link} target="_blank" rel="noopener noreferrer"
                                   key={index} onMouseEnter={() => playAudio(pair.audio)}
                                   onMouseLeave={() => stopAudio(pair.audio)}>
                                    <ImageSrcWrapper overlayText={pair.credit} wrapperClassName={"w-full h-1/10"}>
                                    <div className="bg-cover bg-center h-64"
                                        ...
                                        <span
                                            className="text-m text-deep-brown font-bold vertical-align-bottom">{pair.text}</span>
                                        </div>
