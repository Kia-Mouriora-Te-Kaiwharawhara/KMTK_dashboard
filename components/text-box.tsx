...
function TextBox({
  ...
  if(type == null) {
  return (
    <div className={cn(`9/10 bg-primary-tp text-center p-5 h-auto`, className)} {...props}>
      {text && <span className={cn("text-m text-roto-blue")}>{text}</span>}
    </div>
  )
} else if (type == "dark") {
  return (
    <div className={cn(`9/10 bg-kawakawa-green text-center p-5 h-auto`, className)} {...props}>
